...
public class Triangle {
    ...
    public boolean overlaps(Triangle r) {
        ArrayList<Line> triLines = Line.asLines(this);
        ArrayList<Line> rectLines = Line.asLines(r);
        boolean ov = false;
        for (Line l1 : triLines) {
    ...
    public static ArrayList<Triangle> triangulate(Rectangle rec){
        ArrayList<Triangle> output = new ArrayList<>();
        Vector2 center = new Vector2((rec.width/2)+ rec.x,rec.height/2+rec.y);
        Vector2 a = new Vector2(rec.x,rec.y);
        Vector2 b = new Vector2(rec.x,rec.y+rec.height);
        Vector2 c = new Vector2(rec.x+rec.width,rec.height+rec.y);
        Vector2 d = new Vector2(rec.x+rec.width,rec.y);

        Triangle t1 = new Triangle(a,center,b);
        Triangle t2 = new Triangle(b,center,c);
        Triangle t3 = new Triangle(c,center,d);
        Triangle t4 = new Triangle(d,center,a);
        output.add(t1);
        output.add(t2);
        output.add(t3);
        output.add(t4);
